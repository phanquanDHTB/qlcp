...
import { v4 } from 'uuid';
import { CloudUploadOutlined, EditOutlined, CheckCircleFilled, CheckOutlined } from '@ant-design/icons';
...
const BaseExport = forwardRef(({ entity, onReload }: BaseExportProps, ref) => {
  ...
  function exportSample(payload: any) {
    call({
      ...
      .then((response: any) => {
        ...
        const fileName = `File mẫu ${Title.title[camelCase(entity)]?.toLowerCase()}.xlsx`;
  ...
  return (
    <div className={'base_import'}>
      <Button onClick={() => setIsModalOpen(true)} style={{ marginRight: 10, color: 'red' }} type="text">
        <CloudUploadOutlined />
        Nhập Excel
      </Button>
      <Modal
          ...
          <div>
            <Row>
              <Col span={24}>
                <Controller
                  ...
                  control={control}
                  render={({ field }) => (
                    <Radio.Group {...field} defaultValue={upLoadTypeEnum.THEM_MOI}>
                      <Radio value={upLoadTypeEnum.THEM_MOI} onClick={() => setUploadType(upLoadTypeEnum.THEM_MOI)}>
                        {'Thêm mới'}
                      </Radio>
            ...
            <Row>
              <p>
                ...
                <a style={{ color: 'red' }} onClick={() => handleExportTemplateFile()}>
                  {'tại đây'}
                </a>
              </p>
            </Row>
