...
import title from '@columnTitles/airport';
import { useRef, useState } from 'react';
import { v4 } from 'uuid';
import { deleteRecord } from '../../../../apis/airport/airport';
    ...
    Modal.confirm({
      ...
      onOk: () => {
        deleteRecord(record.id)
          .then(() => {
            message.success(`Xóa thành công sân bay ${record.code}`);
            reloadTable();
          })
          .catch((e) => {
    ...
    if (Object.keys(record).length > 0) {
      return (
        <div className={'infor-wrap'}>
          ...
          <Row>
            <Col span={12}>
              <Col style={{ fontWeight: '600', marginTop: 15 }} span={24}>
                {' '}
                Mô tả:
              </Col>
              <Col span={24} style={{ marginTop: 10 }}>
                <span>{record.country ? record.description : '--'}</span>
              </Col>
            </Col>
          </Row>
        </div>
